...
ratio_dt <- annual_stats_all[
  ,
  .(
    up_to_2003 = mean(exeve_yr[year <= 2003], na.rm = TRUE),
    after_2003 = mean(exeve_yr[year >  2003], na.rm = TRUE)
  ),
  by = .(region, lon, lat)
]
...
ggsave("results/figure/annual_exeve_ratio_kg_lc.pdf", dpi = 600, height = 11.69, width = 11.69, units = "in", bg="white")
