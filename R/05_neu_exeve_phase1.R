...
evap_neu <-
  tabular("data/raw/gleam-v4-1a_e_mm_neuland_198001_202312_025_daily.nc") # this wont be run in windows

evap_copy <- copy(evap_neu)[order(lon, lat)]
evap_copy[, grid_id := .GRP, by = .(lat, lon)]
lookup <- unique(evap_copy[, .(grid_id, lon, lat)])
exeves <- detect_exeve(evap_neu)
...
## Save all outputs
saveRDS(annual_summary_long,      "data/processed/annual_summary_neu.rds")
saveRDS(annual_intensity_long,    "data/processed/annual_intensity_neu.rds")
saveRDS(extreme_wide,             "data/processed/extreme_ratio_annual_neu.rds")
saveRDS(monthly_ratio_summary,    "data/processed/monthly_ratio_neu.rds")
