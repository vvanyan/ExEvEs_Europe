...
setDT(annual_stats_neu)
setDT(annual_stats_med)
setDT(annual_stats_wce)
setDT(masks)
...
ratio_dt <- annual_stats_all[
  ,
  .(
    up_to_2003 = mean(evap_yr[year <= 2003], na.rm = TRUE),
    after_2003 = mean(evap_yr[year >  2003], na.rm = TRUE)
  ),
  by = .(region, lon, lat)
]
...
ggsave("results/figure/annual_evap_ratio.pdf", dpi = 600, height = 11.69, width = 11.69, units = "in", bg="white")
