...
evap_wce <- readRDS("data/tabular/evap_1980_2023_wce.rds")

evap_copy <- copy(evap_wce)[order(lon, lat)]
evap_copy[, grid_id := .GRP, by = .(lat, lon)]
lookup <- unique(evap_copy[, .(grid_id, lon, lat)])
exeves <- detect_exeve(evap_wce)
...
# drop all pixels falling below 34 ( our focus is EUwceiterranean region)
...
saveRDS(annual_stats, "data/processed/annual_stats_wce.rds")
